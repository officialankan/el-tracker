...
- [x] **test water tracking**: import a water CSV file, verify toggle switches between El/H2O modes,
...
- [x] in the patterns page when we select a specific month as a period, the plot "average by month" becomes irrelevant, so we should probably hide that if we are viewing a certain month.
...
- [x] on the import page, add a feature that let's the user detect missing dates within the database
